package org.jboss.resteasy.test.nextgen.cdi.validation;
...
public class ValidationTest
{
   ...
   public void testCorrectValues() throws Exception
   {
      ...
      WebTarget target = ClientBuilder.newClient().target("http://localhost:8080/resteasy-cdi-ejb-test/rest/correct/test/7");
      Response response = target.request().get();
      log.info("status: " + response.getStatus());
      log.info("response: " + response.readEntity(Integer.class));
      Assert.assertEquals(200, response.getStatus());
      Assert.assertTrue(8 == response.readEntity(Integer.class));
      response.close();
   }
   ...
   public void testIncorrectInputValues() throws Exception
   {
      ...
      WebTarget target = ClientBuilder.newClient().target("http://localhost:8080/resteasy-cdi-ejb-test/rest/incorrect/test/17");
      Response response = target.request().get();
      log.info("status: " + response.getStatus());
      List<List<String>> violations = response.readEntity(List.class);
...
//      ClientResponse<?> response = request.get();
      WebTarget target = ClientBuilder.newClient().target("http://localhost:8080/resteasy-cdi-ejb-test/rest/correct/test/10");
      Response response = target.request().get();
      log.info("status: " + response.getStatus());
      List<List<String>> violations = response.readEntity(List.class);
      log.info("result: " + violations);
