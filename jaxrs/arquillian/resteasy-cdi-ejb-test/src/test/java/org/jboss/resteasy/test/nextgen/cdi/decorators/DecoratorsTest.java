...
//      ResteasyProviderFactory.getInstance().setInjectorFactory(new CdiInjectorFactory());
//   }
...
//      Assert.assertEquals(0, id);
      ...
      // Create book.
      Client client = ClientBuilder.newClient().register(BookReader.class);
      WebTarget baseTarget = client.target("http://localhost:8080/resteasy-cdi-ejb-test/rest");
      Book book = new Book("RESTEasy: the Sequel");
      Entity<Book> entity = Entity.entity(book, Constants.MEDIA_TYPE_TEST_XML_TYPE);
      Response response = baseTarget.path("create").request().post(entity);
      log.info("Status: " + response.getStatus());
      assertEquals(200, response.getStatus());
...
//      Assert.assertEquals(book, result);
    ...
    // Retrieve book.
    response = baseTarget.path("book/" + id).request(Constants.MEDIA_TYPE_TEST_XML).get();
    log.info("Status: " + response.getStatus());
    assertEquals(200, response.getStatus());
...
//      response.releaseConnection();
    ...
    // Test order of decorator invocations.
    response = baseTarget.path("test").request().get();
    log.info("Status: " + response.getStatus());
    assertEquals(200, response.getStatus());
   }
}
