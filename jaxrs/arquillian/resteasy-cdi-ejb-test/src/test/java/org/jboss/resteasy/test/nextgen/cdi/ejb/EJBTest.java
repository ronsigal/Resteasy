package org.jboss.resteasy.test.nextgen.cdi.ejb;
...
import org.jboss.resteasy.cdi.ejb.EJBResourceParent;
...
public class EJBTest
{
   ...
   {
      WebTarget target = ClientBuilder.newClient().target("http://localhost:8080/resteasy-ejb-test/verifyScopes/");
      Response response = target.request().get();
      log.info("status: " + response.getStatus());
      log.info("result: " + response.readEntity(Integer.class));
      assertEquals(200, response.getStatus());
      assertEquals(200, response.readEntity(Integer.class).intValue());
   }
   ...
   public void testVerifyInjectionJaxRs() throws Exception
   {
      ...
      WebTarget target = ClientBuilder.newClient().target("http://localhost:8080/resteasy-ejb-test/verifyInjection/");
      Response response = target.request().get();
      log.info("result: " + response.readEntity(Integer.class));
      assertEquals(200, response.getStatus());
      assertEquals(200, response.readEntity(Integer.class).intValue());
   ...
   public void testAsJaxRSResource() throws Exception
   {
      ...
      WebTarget baseTarget = ClientBuilder.newClient().target("http://localhost:8080/resteasy-ejb-test");
      
      // Create book.
      Book book1 = new Book("RESTEasy: the Sequel");
      Response response = baseTarget.path("create").request().post(Entity.entity(book1, Constants.MEDIA_TYPE_TEST_XML));
      log.info("Status: " + response.getStatus());
      assertEquals(200, response.getStatus());
      ...
      response = baseTarget.path("create").request().post(Entity.entity(book2, Constants.MEDIA_TYPE_TEST_XML));
      log.info("Status: " + response.getStatus());
      assertEquals(200, response.getStatus());
      ...
      // Retrieve first book.
      response = baseTarget.path("book/" + id1).request(Constants.MEDIA_TYPE_TEST_XML).get();
      log.info("Status: " + response.getStatus());
      assertEquals(200, response.getStatus());
      ...
      response = baseTarget.path("book/" + id2).request(Constants.MEDIA_TYPE_TEST_XML).get();
      log.info("Status: " + response.getStatus());
      assertEquals(200, response.getStatus());
      ...
      response = baseTarget.path("uses/4").request().get();
      log.info("Status: " + response.getStatus());
      assertEquals(200, response.getStatus());
      response.close();
      
      // Reset counter.
      response = baseTarget.path("reset").request().get();
      log.info("Status: " + response.getStatus());
