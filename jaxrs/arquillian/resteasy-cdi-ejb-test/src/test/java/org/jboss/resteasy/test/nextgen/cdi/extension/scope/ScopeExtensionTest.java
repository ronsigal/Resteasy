package org.jboss.resteasy.test.nextgen.cdi.extension.scope;
...
public class ScopeExtensionTest
{
   ...
   public void testObsolescentScope() throws Exception
   {
      ...
      WebTarget target = ClientBuilder.newClient().target("http://localhost:8080/resteasy-extension-test/rest/extension");
      Response response = target.path("setup").request().get();
      log.info("Status: " + response.getStatus());
      assertEquals(200, response.getStatus());
      response.close();
      response = target.path("test1").request().get();
      log.info("Status: " + response.getStatus());
      assertEquals(200, response.getStatus());
      response.close();
      response = target.path("test2").request().get();
      log.info("Status: " + response.getStatus());
      assertEquals(200, response.getStatus());
      response.close();
   }
