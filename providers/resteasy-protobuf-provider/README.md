...
1. `org.jboss.resteasy.plugins.protobuf.JavaToProtobufGenerator` translates a JAX-RS 
   ...
   Note that the protobuf syntax doesn't support inheritence, so `JavaToProtobufGenerator` uses a designated field to represent
   a parent object.
...
3. For each class discovered by `JavaToProtobufGenerator`, there are two versions:
   ...
   B. the Java class generated from the .proto file by the protobuf compiler. This latter class we refer to as the **javabuf** version.
