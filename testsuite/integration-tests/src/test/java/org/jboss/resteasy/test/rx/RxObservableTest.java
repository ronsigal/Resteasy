...
import org.jboss.resteasy.test.rx.resource.RxObservableScheduledExecutorService;
...
public class RxObservableTest {
   ...
   public static Archive<?> deploy() {
      ...
      war.addClass(RxObservableScheduledExecutorService.class);
      ...
      return TestUtil.finishContainerPrepare(war, null, RxObservableResource.class);
   }
   ...
   public void testHead() throws Exception {
      ...
      observable.subscribe(
         (String o) -> stringList.add(o),
         (Throwable t) -> Assert.fail(t.getMessage()),
         () -> Assert.assertEquals(0, stringList.size())); // HEAD - no body
   }
   ...
   public void testScheduledExecutorService () throws Exception {
      {
         ...
         observable.subscribe(
            ...
            (Throwable t) -> Assert.fail(t.getMessage()),
            () -> {Assert.assertEquals(xStringList, stringList);
                   Assert.assertFalse(RxObservableScheduledExecutorService.used);});
         
      }

      {
         RxObservableScheduledExecutorService executor = new RxObservableScheduledExecutorService();
         ResteasyClient client = ((ResteasyClientBuilder) new ResteasyClientBuilder().scheduledExecutorService(executor)).build();
         client.register(ObservableRxInvokerProvider.class);
         ObservableRxInvoker invoker = client.target(generateURL("/get/string")).request().rx(ObservableRxInvoker.class);
         Observable<String> observable = (Observable<String>) invoker.get();
         stringList.clear();
         observable.subscribe(
            ...
            (Throwable t) -> Assert.fail(t.getMessage()),
            () -> {Assert.assertEquals(xStringList, stringList);
                   Assert.assertTrue(RxObservableScheduledExecutorService.used);});
      }
   }
}