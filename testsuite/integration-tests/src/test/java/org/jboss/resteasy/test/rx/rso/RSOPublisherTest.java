...
//            war.addAsLibraries(jars);
      //      JavaArchive[] jars = Maven.resolver().loadPomFromFile("pom.xml")
//            .resolve("org.jboss.resteasy:resteasy-reactive-streams-operators")
//            .withTransitivity().as(JavaArchive.class);
//      war.addAsLibraries(jars);
//      jars = Maven.resolver().loadPomFromFile("pom.xml")
//            .resolve("org.eclipse.microprofile.reactive-streams-operators:microprofile-reactive-streams-operators-api")
//            .withTransitivity().as(JavaArchive.class);
//      war.addAsLibraries(jars);
//      jars = Maven.resolver().loadPomFromFile("pom.xml")
//            .resolve("org.eclipse.microprofile.reactive-streams-operators:microprofile-reactive-streams-operators-core")
//            .withTransitivity().as(JavaArchive.class);
//      war.addAsLibraries(jars);
//      JavaArchive[] jars = Maven.resolver().loadPomFromFile("pom.xml")
//            .resolve("io.smallrye.reactive:smallrye-reactive-streams-operators")
//            .withTransitivity().as(JavaArchive.class);
//      war.addAsLibraries(jars);
//      jars = Maven.resolver().loadPomFromFile("pom.xml")
//          .resolve("org.reactivestreams:reactive-streams")
...
////            .withTransitivity().as(JavaArchive.class);
////        war.addAsLibraries(jars);
      Archive ar = TestUtil.finishContainerPrepare(war, null, RSOPublisherResourceImpl.class, TestExceptionMapper.class);
      ar.as(ZipExporter.class).exportTo(new File("/tmp/ar.zip"), true);
      return ar;
   }
   ...
   static class TestSubscriber<T> implements Subscriber<T> {
      ...
      TestSubscriber(final Consumer<? super T> onNext, final Consumer<? super Throwable> onError, final Action onComplete) {
         this.onNext = onNext;
